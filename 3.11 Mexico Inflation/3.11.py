...
file_path = r'C:/GitHub/DeepLearning/3.11 Mexico Inflation/M HISTORICAL INFLATION RATES.csv'
df = pd.read_csv(file_path)

# Extract the relevant column (assuming 'Inflation Rate' is the column name)
close_prices = df['Inflation Rate'].values.reshape(-1, 1).astype(float)
...
seq_length = 4  # Reduced for better stability
...
for epoch in range(num_epochs):
    ...
    if (epoch + 1) % 100 == 0:
        print(
            f'Epoch [{epoch+1}/{num_epochs}], Loss: {epoch_loss / len(dataloader):.15f}')
